...
def test_credit_arrangements_accounts_processor():
    from singer.catalog import Catalog
    catalog = Catalog.load(f"{FIXTURES_PATH}/processor_catalog.json")
    client_mock = MagicMock()

    from tap_mambu.tap_mambu_refactor.tap_processors.credit_arrangements_processor import CreditArrangementsProcessor
    processor = CreditArrangementsProcessor(catalog=catalog,
                                            stream_name="credit_arrangements",
                                            client=client_mock,
                                            config=config_json,
